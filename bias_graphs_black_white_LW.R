...
dat$group_new <- ordered(dat$group_new,
                     ...
                     labels = c("Online-only", "Push-to-web", "Concurrent", "Paper-first", "Bonus 20", "Bonus 50"))
...
rel.var_o1 <-  c(rep("female", 6), rep("low", 6), rep("medium", 6), rep("high", 6), 
                      rep("16 to 29", 6), rep("30 to 39", 6), 
                      rep("40 to 49", 6),  rep("50 to 59", 6), rep("> 60", 6))
...
ggdat1 <- data.frame(props = c(props_f, props_e1, props_e2, props_e3, props_a1, props_a2, props_a3, props_a4, props_a5), 
                    ...
                    rel.low = rel.low_o1,
                     ...
                     var = c(rep("female", 6), rep("low", 6), rep("medium", 6), rep("high", 6), 
                               rep("16 to 29", 6), rep("30 to 39", 6), 
                               rep("40 to 49", 6),rep("50 to 59", 6), rep("> 60", 6)))
#ggdat2$ord <- c(rep(1, 6), rep(2, 6), rep(3, 6), rep(4, 6), rep(5, 6), rep(6, 6), rep(7, 6), rep(8, 6))
ggdat1$var2 <- factor(ggdat1$var, levels = c("female", "low", "medium", "high", "16 to 29", "30 to 39", 
                                             "40 to 49", "50 to 59","> 60"))
ggdat1$group <- factor(ggdat1$group, levels = c(  "bonus 50 Euro", "bonus 20 Euro", "online-only","push-to-web", "concurrent","paper-first" ))
...
#### Graphiken push-to-web
jpeg("ptw_bias_female_part1_bw_2.jpg", units="in", width=7, height=5, res=300)
p <- ggplot(subset(ggdat1, ggdat1$group %in% c("paper-first", "concurrent", "push-to-web",  "online-only") & var == "female"), aes(y= (rel.bias ) , x=group, ymin = (rel.low ), ymax = (rel.high)))+
 geom_point(aes(colour=group)) + ylim(-25, 25) +
  ggtitle("Recruitment Survey") +
  #theme(plot.title = element_text(hjust = 0.5))+
  theme_bw() +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, colour = "black"), 
        axis.text = element_text(size = 15, color = "black"), axis.title.x = element_text(size = 20), 
...
jpeg("ptw_bias_age_part1_bw_2.jpg", units="in", width=7, height=7, res=300)
p <- ggplot(subset(ggdat1, ggdat1$group %in% c("paper-first", "concurrent", "push-to-web",  "online-only") & var %in% c("16 to 29", "30 to 39",
    "40 to 49","50 to 59", "> 60")), aes(y= (rel.bias ) , x=group, ymin = (rel.low ), ymax = (rel.high)))+
  geom_point(aes(colour=group)) + ylim(-50, 50) +
  ggtitle("Recruitment Survey") +
 # theme(plot.title = element_text(hjust = 0.5))+
  theme_bw() +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, colour = "black"), strip.background = element_rect(fill="white"), 
        axis.text = element_text(size = 15, color = "black"), axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20))+
  ...
  theme(legend.position="none", strip.text = element_text(color = "black", size = 15)) +
  geom_pointrange(aes(colour=group, linetype=group)) +
  scale_linetype_manual(values=c("solid", "solid", "solid", "solid"))
p + theme(plot.title = element_text(hjust = 0.5, size = 20))
dev.off()
...
jpeg("ptw_bias_edu_part1_bw_2.jpg", units="in", width=7, height=5, res=300)
p <- ggplot(subset(ggdat1, ggdat1$group %in% c("paper-first", "concurrent", "push-to-web",  "online-only") & var %in% c("low", "medium",
                                                                "high")), aes(y= (rel.bias ) , x=group, ymin = (rel.low ), ymax = (rel.high)))+
  geom_point(aes(colour=group)) + ylim(-80, 80) +
  ggtitle("Recruitment Survey") +
#  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw() +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
rel.var_o2 = c(rep("female", 6), rep("low", 6), rep("medium", 6), rep("high", 6), 
               rep("16 to 29", 6), rep("30 to 39", 6), 
               rep("40 to 49", 6),  rep("50 to 59", 6), rep("> 60", 6), 
               rep("single", 6), rep("married", 6), rep("divorced", 6), rep("widowed", 6), 
        rep("german", 6), rep("1", 6),  rep("2", 6),  rep("3", 6),  rep("> 4", 6))


# F?r Familienstand und Nationalit?t erg?nzen
group_inc <- c("bonus 50 Euro", "bonus 20 Euro", "no bonus", "paper-first", "concurrent", "push-to-web")
...
ggdat2 <- data.frame(props = c(props_f,props_e1, props_e2, props_e3, props_a1, props_a2, props_a3, props_a4, props_a5, props_m1, props_m2, props_m3, props_m4, props_g, props_h1, props_h2, props_h3, props_h4), 
                     ...
                     var = c(rep("female", 6), rep("low", 6), rep("medium", 6), rep("high", 6), 
                             rep("16 to 29", 6), rep("30 to 39", 6), 
                             rep("40 to 49", 6),  rep("50 to 59", 6), rep("> 60", 6), rep("single", 6), rep("married", 6), rep("divorced", 6), rep("widowed", 6), 
                             rep("german", 6), rep("1", 6),  rep("2", 6),  rep("3", 6),  rep("> 4", 6)))

ggdat2$var2 <- factor(ggdat2$var, levels = c("female", "low", "medium", "high", "16 to 29", "30 to 39", 
                                             "40 to 49", "50 to 59","> 60", "> 4", "3", "2", "1",  "married", "widowed", "divorced", "single"))
ggdat2$group <- factor(ggdat2$group, levels = c(  "bonus 50 Euro", "bonus 20 Euro", "online-only","push-to-web", "concurrent","paper-first" ))
...
jpeg("ptw_bias_female_online2_bw_2.jpg", units="in", width=7, height=5, res=300)
p <- ggplot(subset(ggdat2, ggdat2$group %in% c( "paper-first", "concurrent", "push-to-web",  "online-only") & var == "female"), 
            aes(y= (rel.bias ) , x=group, ymin = (rel.low ), ymax = (rel.high)))+
  geom_point(aes(colour=group)) + ylim(-25, 25) +
  ggtitle("Registration Survey") +
  theme_bw() +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, colour = "black"),
        axis.text = element_text(size = 15, colour = "black"), axis.title.x = element_text(size = 20), 
        axis.title.y = element_text(size = 20))+
  ylab("relative bias in proportion of females") + 
  xlab("") +
  scale_color_manual(values=c("black", "black",  "black" , "black")) +
...
jpeg("ptw_bias_age_online2_bw_2.jpg", units="in", width=7, height=7, res=300)
p <- ggplot(subset(ggdat2, ggdat2$group %in% c( "paper-first", "concurrent", "push-to-web",  "online-only") & var %in% c("16 to 29", "30 to 39",
                                                                                                                    "40 to 49", "50 to 59", "> 60")), aes(y= (rel.bias ) , x=group, ymin = (rel.low ), ymax = (rel.high)))+
  geom_point(aes(colour=group)) + ylim(-50, 50) +
  ggtitle("Registration Survey") +
  # theme(plot.title = element_text(hjust = 0.5))+
  ...
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5, colour = "black"),
        axis.text = element_text(size = 15, colour = "black"), axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20), strip.background = element_rect(fill="white"))+
  ylab("relative bias in age categories") + 
  xlab("") +
  facet_grid(rows = vars(var))+
  scale_color_manual(values=c("black", "black",  "black" , "black")) +
...
jpeg("ptw_bias_edu_online2_bw_2.jpg", units="in", width=7, height=5, res=300)
p <- ggplot(subset(ggdat2, ggdat2$group %in% c( "paper-first", "concurrent", "push-to-web",  "online-only") & var %in% c("low", "medium",
                                                                                                                    "high")), aes(y= (rel.bias ) , x=group, ymin = (rel.low ), ymax = (rel.high)))+
  geom_point(aes(colour=group)) + ylim(-80, 80) +
  ggtitle("Registration Survey") +
  #  theme(plot.title = element_text(hjust = 0.5))+
...
p <- ggplot(subset(ggdat2, ggdat2$group %in% c( "paper-first", "concurrent", "push-to-web",  "online-only") & var %in% c("single", "married", "widowed",
...
p <- ggplot(subset(ggdat2, ggdat2$group %in% c( "paper-first", "concurrent", "push-to-web",  "online-only") & var %in% c("> 4", "3", "2", "1")), aes(y= (rel.bias ) , x=group, ymin = (rel.low ), ymax = (rel.high)))+
  geom_point(aes(colour=group)) + ylim(-50, 50) +
  ggtitle("Registration Survey") +
  #  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw() +
...
p <- ggplot(subset(ggdat2, ggdat2$group %in% c( "paper-first", "concurrent", "push-to-web",  "online-only") & var == "german"), aes(y= (rel.bias ) , x=group, ymin = (rel.low ), ymax = (rel.high)))+
  geom_point(aes(colour=group)) + ylim(-25, 25) +
  ggtitle("Registration Survey") +
  #  theme(plot.title = element_text(hjust = 0.5))+
  theme_bw() +
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
        axis.text = element_text(size = 15, colour = "black"), axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20))+
...
max.dat <- data.frame(rel.bias1 = unlist(rel.bias.max1), rel.bias2 = unlist(rel.bias.max2), 
                      group=c(rep("bonus 50 Euro",6 ), rep("bonus 50 Euro",6 ),rep("paper-first",6 ),
                      rep("concurrent",6 ), rep("push-to-web",6 ), rep("online-only",6 )))

jpeg("ptw_abs_rel_biases_online1_2.jpg", units="in", width=7, height=5, res=300)
p <- ggplot(subset(rel.bias.dat_o1, rel.bias.dat_o1$group %in% c("paper-first", "concurrent", "push-to-web", "online-only" )),
            aes(x=group, y=bias, fill =group)) + 
  geom_boxplot() + 
  scale_y_continuous(name = "absolute relative bias", limits = c(0,68), labels = function(x) paste0(x/1, "%"))+
  xlab("")+
  ggtitle("Recruitment Survey")+
 # scale_fill_brewer(palette=c("darkgreen", "darkred",  "darkblue" , "grey30"))+
  ...
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(size = 0.5,colour = "black"), axis.text = element_text(size = 15, colour = "black"), 
...
jpeg("ptw_abs_rel_biases_online2_2.jpg", units="in", width=7, height=5, res=300)
p <- ggplot(subset(rel.bias.dat_o2, rel.bias.dat_o2$group %in% c("paper-first", "concurrent", "push-to-web", "online-only" )), aes(x=group, y=bias, fill=group)) + 
  geom_boxplot()+
  scale_y_continuous(name = "absolute relative bias", labels = function(x) paste0(x/1, "%"))+
  xlab("")+
  ggtitle("Registration Survey")+
  theme_bw() +
  scale_fill_manual(values=c("grey70","grey70","grey70","grey70" )) +
